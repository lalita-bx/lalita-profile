...
const Experience = () =>{
    ...
    return (
        <div class="flex text-left my-5 card-exp overflow-auto">
            <ol class="relative h-fit border-l border-gray-300 dark:border-gray-700">   
                {data.map(({ label, time, desc }) => (
                    <li key={label} value={desc} class="mb-8 ml-4">
                        <div class="absolute w-3 h-3 bg-gray-300 rounded-full mt-2 -left-1.5 border border-white dark:border-gray-900 dark:bg-gray-700"></div>
                        <time class="mb-1 text-sm font-normal leading-none text-gray-600 dark:text-gray-700">{time}</time>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{label}</h3>
                        <p class="mb-3 text-base font-normal text-left md:text-justify text-gray-700 dark:text-gray-400">{desc}</p>
